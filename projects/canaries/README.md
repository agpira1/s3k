...
| `monitor.ld` | Physical region for `monitor`. Must not overlap another application. |
| `monitor/main.c` | Entry point for `monitor`. Runs once app0 grants it time and resumes it. |
| `vuln.ld` | Physical region for `vuln`. Must not overlap another application. |
| `vuln/main.c` | Entry point for `vuln`. Runs once app0 grants it time and resumes it. |
...
`make new APPS=2` already did all of this:
the linker scripts, the `main.c` files, the `APPS` list,
`S3K_PROC_CNT`, and the grants in `app0/main.c`. It differs
from the steps below in one place: it carves a slice of
`HART0_TIME` with `util_grant_time` rather than moving a whole
hart, so the generated project also runs on single-hart
`qemu_virt`. The steps are kept because they still apply to an
application added by hand.
